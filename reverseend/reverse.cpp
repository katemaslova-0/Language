...
void PrintKeyword (Node * current, FILE * fp)
{
    ...
    switch(current->value.keyword)
    {
        ...
        case kContinue: fprintf(fp, "óäĺđćč-ěĺí˙");     break;
        case kBreak:    fprintf(fp, "ďĺđĺńňóďč-ďîđîă"); break;
        case kReturn:   fprintf(fp, "âĺđíóëń˙-âîëęîě"); break;
...
void PrintFuncInit (Node * current, FILE * fp)
{
    ...
    fprintf(fp, "˙-ďîęŕćó-ňĺáĺ-ěŕđřđóň ");
...
void PrintElse (Node * current, FILE * fp)
{
    ...
    fprintf(fp, "ńďĺřčň-â-äđóăóţ-ńňîđîíó\n{\n");
    PrintLine(current->right, fp);
    fprintf(fp, "}");
}
...
void PrintVarEq (Node * current, FILE * fp)
{
    ...
    fprintf(fp, "çíŕé-ćĺ %s = ", current->value.name.name);
